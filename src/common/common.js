...
export async function getAIUrls() {
  ...
  motores.forEach(model => {
    if (model.id && model.web) {
      AI_URLS[model.id] = {web1 : model.web, web2: model.web2, waitSeconds: model.segundos};
    }
  });
  return AI_URLS;
}
...
export async function openAIWithPrompt(prompt, label) {
  // Verificar si el prompt original está vacío
  const isEmptyPrompt = !prompt || prompt.trim() === "";
  console.log("🔍 Debug openAIWithPrompt - Prompt original:", prompt, "isEmpty:", isEmptyPrompt);
